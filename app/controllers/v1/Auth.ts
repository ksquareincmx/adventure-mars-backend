...
export class AuthController extends Controller {
  ...
  routes(): Router {
    /**
      ...
      @apiParam {Object} credentials                  Login Credentials
      @apiParam {String} credentials.email            User Email
      @apiParam {String} credentials.password         User Password

      @apiSuccess {Object} credentials                Success Credentials
      @apiSuccess {String} credentials.token          JWT Token
      @apiSuccess {Number} credentials.expires        Token expiration time (unix timestamp in ms)

      @apiSuccess {Object} credentials.refresh_token              JWT Refresh Token data
      @apiSuccess {String} credentials.refresh_token.token        JWT Refresh Token
      @apiSuccess {String} credentials.refresh_token.token        JWT Refresh Token
      @apiSuccess {Number} credentials.refresh_token.expires      Refresh Token expiration time (unix timestamp in ms)
      @apiSuccess {Number} credentials.refresh_token.expires_in   Refresh Token expiration time (remaining ms)

      @apiSuccess {Object} credentials.user           User details
      @apiSuccess {Number} credentials.user.id        User id
      @apiSuccess {String} credentials.user.name      User name
      @apiSuccess {String} credentials.user.email     User email
      @apiSuccess {String} credentials.user.role      User role ("scout", "leader", "admin")
      @apiSuccess {Number} credentials.user.unitId    User unit id
    */
    ...
    */
    ...
    /**
      @api {post} /api/v1/auth/register/ Register a new scout user
      @apiPermission none (only registers scout users)
      @apiName Register
      @apiGroup Auth

      @apiParam {Object} credentials                  Register Credentials
      @apiParam {String} credentials.name             User Name
      @apiParam {String} credentials.email            User Email
      @apiParam {String} credentials.password         User Password
      @apiParam {Number} credentials.unitId           User Unit Id
    ...
    */
    ...
    /**
      @api {post} /api/v1/auth/registerleader/ Register a new leader user
      @apiPermission admin
      @apiName RegisterLeader
      @apiGroup Auth
      ...
      @apiSuccess {Number} credentials.user.unitId    User unit id
    */
    this.router.post("/registerleader", validateJWT("access"), filterRoles(["admin"]), (req, res) =>
      this.register(req, res, "leader")
    );

    /**
      @api {get} /api/v1/auth/reset?token=[reset_token] Recover Password, Start the Reset Password flow
    ...
    */
    ...
    /**
      @api {post} /api/v1/auth/reset/ Recover Password, Request password recovery or set new password
      @apiDescription
      ...
      @apiPermission none (requires a valid reset token for setting new password)
      @apiName PostReset
      @apiGroup Auth

      @apiHeader { String } Content-Type Application/Json

      @apiParam (Recovery request)  {Object} body           Variable Data
      @apiParam (Recovery request)  {String} body.email     User Email

      @apiParam (Set password)      {Object} body           Variable Data
      @apiParam (Set password)      {String} body.token     Recovery token
      @apiParam (Set password)      {String} body.password  New password to set
    ...
    */
    ...
    /**
      ...
      @apiParam {Object} data                         Change password data
      @apiParam {String} data.email                   User Email
      @apiParam {String} data.oldPass                 User old Password
      @apiParam {String} data.newPass                 User new Password
    ...
    */
    ...
    /**
      @api {post} /api/v1/auth/refresh/ Refresh JWT token
  ...
  protected getCredentials(user: any): any {
    ...
    let credentials = {
      ...
      user: _.pick(user, ["id", "name", "email", "role", "unitId"])
    };
    return credentials;
  }
  ...
  private sendEmailNewPassword(user: any, token: string, name?: string): Promise<any> {
    ...
    return mailer
      .sendEmail(user.email, subject, "password_recovery", "en", {
  ...
  private sendEmailPasswordChanged(user: any, name?: string): Promise<any> {
    ...
    return mailer
      .sendEmail(user.email, subject, "password_changed", "en", {
        name: name || user.email
      })
      .then(info => {
  ...
  register(req: Request, res: Response, role: "scout" | "leader" | "admin" = "scout") {
    let newUser = {
      name: req.body.name,
